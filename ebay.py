...
def scrape_staples_selenium(product_name: str):
    ...
    try:
        search_url = f"https://www.ebay.ca/sch/i.html?_nkw={product_name}"
    ...
    with open("EBAYSoup_output.txt", "w", encoding="utf-8") as f:
        f.write(soup.prettify())

    num = 1
    results = []
    for li_tag in soup.select("li.s-card s-card--horizontal s-card--dark-solt-links-blue"):
        ...
        href = li_tag.get("href", "")
        link = f"https://www.staples.ca{href}" if href.startswith("/") else href

        # Try to find the price near this link (look upward or in the same container)
        card = li_tag.find_parent("div", class_="product-thumbnail")
...
if __name__ == "__main__":
    ...
    while searching == 'y':
        ...
        products = scrape_staples_selenium(product_desc)
        ...
        searching = user_input[:1].lower()
        while searching not in ('y', 'n'):
            searching = input("Perform another search? (y/n): ")[:1].lower()
